 ...
 *
 * Checks if an existing tutorial's chapters are outdated by comparing
 * the README.md "Based On Commit" against the current git HEAD.
 */

import { Type } from "@sinclair/typebox";
import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";
import { README_FILENAME, CHAPTERS_FILENAME } from "../constants.js";
...
export function registerCheckTutorialDriftTool(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			if (!sourceDir) {
				return {
					content: [{ type: "text", text: `No source directory found. Please provide either a ${README_FILENAME} with "Source Location" or pass sourceDir parameter.` }],
			...
			if (!currentCommit) {
				return {
					content: [{ type: "text", text: `Could not get current git commit from ${sourceDir}. Is this a git repository?` }],
