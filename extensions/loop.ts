 ...
 * ## Usage
 *
 * Start a loop with one of these commands:
 *
 *   /loop tests        - Loop until all tests pass
 *   /loop self         - Loop until the agent decides it's done
 *   /loop custom <condition> - Loop until a custom condition is satisfied
 *
 * Examples:
 *   /loop custom all functions are under 30 lines
 *   /loop custom code has no TODO comments remaining
 *   /loop custom API documentation is complete
 *
 * Without arguments, an interactive selector will appear to choose the mode.
 *
 * ## How It Works
 *
 * 1. When you start a loop, the extension injects a follow-up prompt after each
 *    agent turn ends (unless you interrupt).
 *
 * 2. The agent continues working toward the breakout condition.
 *
 * 3. When satisfied, the agent calls the `signal_loop_success` tool to end the loop.
 *
 * 4. You can manually break a loop by saying "break the loop" or if the agent gets
 *    aborted, you'll be prompted to exit the loop.
 *
 * ## Loop Modes
 *
 * - **tests**: Use when you want the agent to keep running tests until they pass.
 *   The agent should call signal_loop_success when all tests are green.
 *
 * - **self**: Use when you want the agent to decide when it's done. The agent
 *   has a subjective goal (e.g., "improve the code quality") and calls
 *   signal_loop_success when it feels finished.
 *
 * - **custom**: Use when you have a specific, observable condition. Be specific:
 *   "all TODOs are addressed" works better than "improve the codebase".
 *
 * ## Tips
 *
 * - Custom conditions work best when they're observable and unambiguous.
 *   "no linter errors" is good. "code looks nice" is bad.
 *
 * - For complex goals, consider using /loop self and describing your goals in
 *   the initial prompt before starting the loop.
 *
 * - The loop persists across session switches and restarts, so you can pick
 *   up where you left off.
 *
 * - State is preserved through context compaction, so loops survive even when
 *   the context gets summarized.
 */
