 ...
 * Uses `notify-send` via D-Bus — works on any Linux/BSD desktop with a notification
 * daemon (dunst, mako, GNOME Notifications, etc.).
 *
 * Requires `notify-send` to be available in PATH.
 */
...
const sendNotification = (title: string, body: string): void => {
	execFileSync("notify-send", [title, body, "-a", "pi", "-u", "normal"], {
		stdio: "ignore",
	});
};
